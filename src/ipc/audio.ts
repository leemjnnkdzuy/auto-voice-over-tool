...
export const setupAudioIpc = () => {
	...
	ipcMain.on("transcribe-audio", (event, projectPath, engine, language) => {
		transcribeAudio(
			projectPath,
			(progress) => {
				safeSend(event.sender, "transcript-progress", progress);
			},
			engine || "whisper-cpu",
			language || "auto",
		).then((result) => {
			safeSend(event.sender, "transcript-complete", result);
		});
	});
		...
		if (fs.existsSync(originalDir)) {
			...
			if (audioFile) {
				...
				const buffer = fs.readFileSync(
					path.join(originalDir, audioFile),
				);
				return {
	...
	ipcMain.on(
		...
		async (event, projectPath: string, lang: string) => {
			try {
				...
				if (!fs.existsSync(srtPath)) {
					safeSend(event.sender, "audio-generate-progress", {
						status: "error",
						progress: 0,
				...
				const results = await generateAllAudio(
					...
					outputDir,
					(p) => {
						safeSend(event.sender, "audio-generate-progress", p);
					},
					1, // sequential
				);
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				const success = await generateAudioSegment(
					entry.text,
