...
export const setupVideoIpc = () => {
    ...
    ipcMain.on("import-local-video", (event, filePath: string, projectPath: string) => {
        ...
        event.sender.send("import-local-progress", { step: "copying", progress: 0, detail: "Đang copy video..." });
        ...
        event.sender.send("import-local-progress", { step: "copying", progress: 50, detail: "Copy video xong. Đang tách audio..." });

        const { getFfmpegPath } = require("../services/EnvironmentService");
        ...
        proc.stderr.on("data", (data: Buffer) => {
            ...
            if (durationMatch && timeMatch) {
                const totalSec = parseInt(durationMatch[1]) * 3600 + parseInt(durationMatch[2]) * 60 + parseInt(durationMatch[3]);
                ...
                if (totalSec > 0) {
                    ...
                    event.sender.send("import-local-progress", { step: "extracting", progress: 50 + pct * 0.5, detail: `Đang tách audio... ${pct}%` });
                }
            }
        });
        ...
        proc.on("close", (code: number) => {
            if (code === 0) {
                event.sender.send("import-local-progress", { step: "done", progress: 100, detail: "Hoàn tất!" });
